...
|------------|--------|---------|
| Stable sqlite-vec `vec0 MATCH` is exact exhaustive KNN, not ANN | Retrieval still scales linearly, but internal SIMD top-K avoids the slow scalar SQL scan. Unsupported substring/tag filters retain scalar fallback. | Upstream: https://github.com/asg017/sqlite-vec |
